...
// improve mail appearance with html template
...
export async function sendEmail({ to, cc, bcc, subject, html, attachments = [] } = {}) {
    ...
    let info = await transporter.sendMail({
        from: `"Exam" <${process.env.GMAIL}>`, // sender address
        to,
        cc,
        bcc,
        subject,
...
export const sendConfirmationCode = ({email,subject} = {}) =>{

  // body of email (contain confirmation code)
  const code = nanoid(5);
  const html = htmlTemplate(code);

  // send email to user to confirm his email
  sendEmail({ to: email, subject, html });

  return code // return confirmation code to store it into DB
}
